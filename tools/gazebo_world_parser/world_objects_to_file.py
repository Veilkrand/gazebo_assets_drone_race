...
if __name__=='__main__' :
	...
	for model in models:
		name = model.attributes['name'].value 
		if name.startswith( _object_name):
			model_object = {}
			model_object['name'] = name
			#print name
			model_object['id'] = get_id_text_from_name(name,_regex)
			for t in model.childNodes:
				if t.nodeName == 'pose':
					pose_text = t.firstChild.nodeValue
					model_object['pose'] = parse_pose(pose_text)
					break
			print name, model_object['id'], model_object['pose']
